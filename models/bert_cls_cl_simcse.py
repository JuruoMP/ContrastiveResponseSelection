...
class BertCls(nn.Module):
    ...
    def forward(self, batch_data):
        ...
        def get_bert_output(name, source_name=None):
            if bert_output_cache.get(name) is not None:
                cls_logits = bert_output_cache.get(name)
            else:
                if not source_name:
                    source_name = name
                batch = batch_data[name]
                bert_outputs = self._model(
        ...
        if original_response_selection or original_contrastive:
            ...
            if original_contrastive:
                cls_logits_aug = get_bert_output('augment', source_name='original')[0]
        ...
        if self.training and (new_response_selection or new_contrastive or supervised_contrastive):
            ...
            if new_contrastive:
                ...
                cls_logits_contras_aug = get_bert_output('contras_aug', source_name='contras')[0]
