...
from ..pretrained_common.activations import get_activation
from .configuration_electra import ElectraConfig
from ..pretrained_common.file_utils import add_start_docstrings, add_start_docstrings_to_callable
from ..bert.modeling_bert import BertEmbeddings, BertEncoder, BertLayerNorm, BertPreTrainedModel
...
def load_tf_weights_in_electra(model, config, tf_checkpoint_path, discriminator_or_generator="discriminator"):
    ...
    for name, array in zip(names, arrays):
        ...
        try:
            ...
            if name.startswith("electra/encoder/transformer"):
                name = name.replace("electra/encoder/transformer/", "electra/encoder/")
...
class ElectraForPreTrainingNSP(ElectraPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    Examples::
        ...
        discriminator_sequence_output = discriminator_hidden_states[0]
        cls_output = discriminator_sequence_output[:, 0, :]
        ...
        if labels is not None:
            ...
            nsp_loss_fct = nn.CrossEntropyLoss(ignore_index=-1)
            nsp_loss = nsp_loss_fct(self.seq_relationship(cls_output).view(-1, 2),
                                    next_sentence_label.view(-1))

            output = (loss, nsp_loss) + output
...
class ElectraForMaskedLM(ElectraPreTrainedModel):
    ...
    Returns:
        ...
        if masked_lm_labels is not None:
            loss_fct = nn.CrossEntropyLoss(ignore_index=-1)  # -100 index = padding token # mlm_loss
