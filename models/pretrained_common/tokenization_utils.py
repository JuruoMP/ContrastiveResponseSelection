...
from transformers.file_utils import cached_path, hf_bucket_url, is_remote_url, is_tf_available, is_torch_available
    ...
    def encode_plus(
        ...
        def get_input_ids(text):
            if isinstance(text, str):
                return self.convert_tokens_to_ids(self._tokenize(text, **kwargs))
            elif isinstance(text, (list, tuple)) and len(text) > 0 and isinstance(text[0], str):
                # tokens = []
                # for token in text:
                #     tokens.extend(self._tokenize(token, **kwargs))
