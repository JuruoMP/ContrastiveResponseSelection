...
class BertCls(nn.Module):
    def __init__(self, hparams):
        ...
        self._classification3 = nn.Sequential(
            nn.Dropout(p=1 - self.hparams.dropout_keep_prob),
            nn.Linear(self.hparams.bert_hidden_dim, 1)
        )
    ...
    def forward(self, batch_data):
        ...
        if self.training:  # training
            ...
            new_response_selection, new_contrastive = True, False
        ...
        logits = torch.Tensor([0]).to(batch_data['original']['res_sel']['anno_sent'].device)
        ...
        if self.training and (new_response_selection or new_contrastive or supervised_contrastive):
            ...
            if new_response_selection:
                logits_contras = self._classification(cls_logits_contras).squeeze(-1)  # bs, 1
                logits_contras2 = self._classification2(cls_logits_contras).squeeze(-1)
                logits_contras3 = self._classification3(cls_logits_contras).squeeze(-1)
                cls_logits_sample = get_bert_output('sample')
                logits_sample = self._classification(cls_logits_sample).squeeze(-1)  # bs, 1
                logits_sample2 = self._classification2(cls_logits_sample).squeeze(-1)
                logits_sample3 = self._classification3(cls_logits_sample).squeeze(-1)
                logits = [torch.cat((logits_contras, logits_sample), dim=0),
                          torch.cat((logits_contras2, logits_sample2), dim=0),
                          torch.cat((logits_contras3, logits_sample3), dim=0)]
                labels = [torch.cat((batch_data['contras']["res_sel"]["label2"][:, 0], batch_data['sample']["res_sel"]["label2"][:, 0]), dim=0),
                          torch.cat((batch_data['contras']["res_sel"]["label2"][:, 1], batch_data['sample']["res_sel"]["label2"][:, 1]), dim=0),
                          torch.cat((batch_data['contras']["res_sel"]["label2"][:, 2], batch_data['sample']["res_sel"]["label2"][:, 2]), dim=0)]
                for i in range(3):
                    loss = self._criterion(logits[i], labels[i].float())
                    mask = labels[i] == -1
                    loss = loss.masked_fill(mask, 0).mean()
                    res_sel_loss_list.append(loss)
