...
class TrainingInstance(object):
    ...
    def __init__(self, tokens, segment_ids, masked_lm_positions, masked_lm_labels,
                 is_random_next, contras_tokens=None, contras_segment_ids=None):
        self.tokens = tokens
        self.segment_ids = segment_ids
        self.is_random_next = is_random_next
        self.masked_lm_positions = masked_lm_positions
        self.masked_lm_labels = masked_lm_labels
        self.contras_tokens = contras_tokens
        self.contras_segment_ids = contras_segment_ids
...
class CreateBertPretrainingData(object):
    ...
    def create_training_instances(self, input_file, max_seq_length,
        ...
        with open(input_file, "r", encoding="utf=8") as fr_handle:
            for line in tqdm(fr_handle):
                ...
                if len(line) == 0:
                    ...
                    # if document_cnt % 5000 == 0: break  # todo: delete here
        ...
        self.feature_keys = ["input_ids", "attention_mask", "token_type_ids",
                             ...
                             "contras_input_ids", "contras_attention_mask", "contras_token_type_ids"]
        ...
        while i < len(document):
            ...
            if i == len(document) - 1 or current_length >= target_seq_length:
                if current_chunk:
                    ...
                    if len(current_chunk) == 1 or rng.random() < 0.5:
                        ...
                        if a_end < len(current_chunk) - 1:
                            for j in range(a_end, len(current_chunk)):
                                contras_tokens_b.extend(current_chunk[j])
                            aaa = 1
                        else:
                            for _ in range(10):
                                random_document_index = rng.randint(0, len(all_documents) - 1)
                            ...
                            random_start = rng.randint(0, len(random_document) - 1)
                            for j in range(random_start, len(random_document)):
                                contras_tokens_b.extend(random_document[j])
                                if len(contras_tokens_b) >= target_b_length:
                                    break
                    # Actual next
                    else:
                        is_random_next = False
                    ...
                    self.truncate_seq_pair(tokens_a, contras_tokens_b, max_num_tokens, rng)

                    assert len(tokens_a) >= 1
                    assert len(tokens_b) >= 1 and len(contras_tokens_b) >= 1
                    ...
                    for token in tokens_a:
                        tokens.append(token)
                        segment_ids.append(0)
                        contras_tokens.append(token)
                        contras_segment_ids.append(0)
                    tokens.append("[SEP]")
                    contras_tokens.append('[SEP]')
                    segment_ids.append(0)
                    contras_segment_ids.append(0)
                    for token in tokens_b:
                        tokens.append(token)
                        segment_ids.append(1)
                    for token in contras_tokens_b:
                        contras_tokens.append(token)
                        contras_segment_ids.append(1)
                    tokens.append("[SEP]")
                    segment_ids.append(1)
                    contras_tokens.append('[SEP]')
                    contras_segment_ids.append(1)
                    ...
                    instance = TrainingInstance(
                        ...
                        contras_tokens=contras_tokens,
                        contras_segment_ids=contras_segment_ids)
    ...
    def instance_to_example_feature(self, instances, max_seq_length, max_predictions_per_seq):
        for instance in instances:
            ...
            contras_input_ids = self._bert_tokenizer.convert_tokens_to_ids(instance.contras_tokens)
            contras_input_mask = [1] * len(contras_input_ids)
            contras_segment_ids = list(instance.contras_segment_ids)

            assert len(input_ids) <= max_seq_length and len(contras_input_ids) <= max_seq_length

            while len(input_ids) < max_seq_length:
                input_ids.append(0)
                input_mask.append(0)
                segment_ids.append(0)
            while len(contras_input_ids) < max_seq_length:
                contras_input_ids.append(0)
                contras_input_mask.append(0)
                contras_segment_ids.append(0)

            assert len(input_ids) == len(contras_input_ids) == max_seq_length
            assert len(input_mask) == len(contras_input_mask) == max_seq_length
            assert len(segment_ids) == len(contras_segment_ids) == max_seq_length
            ...
            self.all_doc_feat_dict["contras_input_ids"].append(contras_input_ids)
            self.all_doc_feat_dict["contras_attention_mask"].append(contras_input_mask)
            self.all_doc_feat_dict["contras_token_type_ids"].append(contras_segment_ids)
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--output_file", dest="output_file", type=str, required=True,
                            help="Output example pkl. e.g., ./data/ubuntu_corpus_v1/ubuntu_post_training_cl.hdf5")
