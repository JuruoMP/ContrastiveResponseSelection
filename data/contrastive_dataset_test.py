...
class ContrastiveResponseSelectionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            positive_example.label2 = (1, 0, 0)
            negative_example.label2 = (0, 0, 1)
            ...
            positive_example_contras = InputExamples(
                ...
                seq_lengths=([len(x) for x in dialogue[st_turn:ed_turn + 1]], len(positive_response)), label2=(1, 0, 0)
            )
            less_positive_example_contras = InputExamples(
                utterances=dialogue[st_turn:ed_turn + 1], response=less_positive_response, label=0,
                seq_lengths=([len(x) for x in dialogue[st_turn:ed_turn + 1]], len(less_positive_response)), label2=(0, 1, 0)
            )
            negative_example_contras = InputExamples(
                utterances=dialogue[st_turn:ed_turn + 1], response=negative_response, label=0,
                seq_lengths=([len(x) for x in dialogue[st_turn:ed_turn + 1]], len(negative_response)), label2=(0, 0, 1)
            )
            ...
            positive_example_contras_aug = InputExamples(
                ...
                seq_lengths=([len(x) for x in dialogue[st_turn:ed_turn + 1]], len(positive_response_aug)), label2=(-1, -1, -1)
            )
            less_positive_example_contras_aug = InputExamples(
                ...
                seq_lengths=([len(x) for x in dialogue[st_turn:ed_turn + 1]], len(less_positive_response_aug)), label2=(-1, -1, -1)
            )
            negative_example_contras_aug = InputExamples(
                utterances=dialogue[st_turn:ed_turn + 1], response=negative_response_aug, label=-1,
                seq_lengths=([len(x) for x in dialogue[st_turn:ed_turn + 1]], len(negative_response_aug)), label2=(-1, -1, -1)
            )
